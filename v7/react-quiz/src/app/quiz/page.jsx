    ...
    <section className="w-full px-8 h-3/5 mt-12 flex flex-col justify-between">
      ...
      ? <>
          ...
          {questions[currentQuestion].answers.map((answer, index) =>
            <button key={index} onClick={() => handleNextQuestion(selectedAnswer)} className="w-full hover:opacity-90 rounded-md text-xs text-white font-medium border-b-4 bg-red-500 border-red-600">
              {answer}
            </button>
          )}
        </div>
      ...
      : <>
          ...
          <Link href="/"><button>Tillbaka till start</button></Link>
      </>
    }
    </section>
  )
